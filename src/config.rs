...
pub fn init_config() -> Result<Config, String> {
  ...
  let provider_name = resolve_string(args.enhance_provider, "ACE_TOOL_ENHANCE_PROVIDER", "remote");
  let provider_kind = EnhanceProviderKind::parse(&provider_name)
    .ok_or_else(|| format!("Invalid provider: {} (expected remote|codex)", provider_name))?;
  let codex_cmd = resolve_string(args.codex_cmd, "ACE_TOOL_CODEX_CMD", "codex");
  let codex_reasoning_effort = resolve_string(args.codex_reasoning_effort, "ACE_TOOL_CODEX_REASONING_EFFORT", "low");
  let enhance_timeout_override = resolve_u64_override(args.enhance_timeout_sec, "ACE_TOOL_ENHANCE_TIMEOUT_SEC", 10, 600);
  ...
  Ok(Config {
    ...
    ui_timeout_sec: resolve_u64(args.ui_timeout_sec, "ACE_TOOL_UI_TIMEOUT_SEC", 8 * 60, 30, 3600),
  })
}
