...
impl IndexManager {
        ...
        if !blobs_to_upload.is_empty() {
            ...
            while !pending.is_empty() && retry_round < max_retry_rounds && fatal_error.is_none() {
                ...
                while idx < batches.len() {
                    let concurrent =
                        &batches[idx..std::cmp::min(idx + strategy.concurrency, batches.len())];
                        ...
                        match handle.await {
                            Ok(Ok(result)) => {
                                if result.success {
                                    ...
                                    fatal_error = Some(
                                        result.error.unwrap_or_else(|| "fatal error".to_string()),
                                    );
                                } else {
    ...
    async fn collect_files(&self) -> Vec<Blob> {
        ...
        while let Some(entry) = walker.next() {
            ...
            let file_blobs =
                split_file_content(&relative_path, &clean_content, self.max_lines_per_blob);
                ...
                Err(err) => {
                    if let Some(status) = err.status() {
                        ...
                        if status.as_u16() == 403 {
                            return Err(
                                "访问被拒绝，Token 可能已被官方禁用，请联系服务提供商".to_string()
                            );
                        }
                        if status.as_u16() == 502 {
                            return Err("服务器网关错误 (502)，ACE 服务可能暂时不可用，请稍后重试"
                                .to_string());
                        }
                        if status.as_u16() == 503 {
                            return Err("服务暂时不可用 (503)，ACE 服务可能正在维护，请稍后重试"
                                .to_string());
                        }
                    ...
                    if message.contains("certificate") || message.contains("tls") {
                        return Err(
                            "SSL 证书验证失败，请检查 ACE_BASE_URL 配置是否正确，或联系服务提供商"
                                .to_string(),
                        );
                    }
