...
use mcp::{schemas, McpLogger, McpServer, ToolHandler};
...
fn handle_enhance_prompt(
  ...
  let current_prompt = match runtime.block_on(enhancer.enhance(prompt, history)) {
    Ok(text) => text,
    Err(err) => {
  ...
  match run_prompt_session(&current_prompt, timeout, continue_cb) {
    SessionAction::UseEnhanced(content) => Ok(content),
    SessionAction::UseOriginal => Ok(prompt.to_string()),
    SessionAction::EndConversation => Ok("__END_CONVERSATION__".to_string()),
    SessionAction::Timeout => Ok(format!(
      "Enhancement timed out (8 minutes). Using original prompt: {}",
      prompt
    )),
  }
